...
語を 1 つ指摘されて直しても、**同じ原則に反する別の語が残ります。** あるレビューコメントの下書きでは、「回る」を指摘されて直した後に「落ちる」「走る」「通る」「出る」が残り、それを直した後に「動かす」が残りました。人間が 3 回読み直しています。これらはすべて、プログラムやデータを主語にして人や物の動きを表す動詞を使っているという**同じ種類**です。
...
- **一覧の網羅の保証** — 原則に照らした判断なので、読みの深さに依存します。走査の回数と各回の結果を報告します。
