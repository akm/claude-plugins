...
（クローズ済み・マージ済みは取得クエリの `--state=open` で対象から外れる）。
...
更新（PR 本文の書き直し、レビューコメントの追加）も検知するため。
...
- **前回からの変更**（`status` が `updated` のときだけ。新規では出さない）
  ...
  - `body_diff` が空、または `body_diff_available` が `false` なら**本文には言及しない**。
...
- `priority` が `ignore` の PR は自動で1行にまとめられる。並び替えも自動。
...
壊れた蓄積データを勝手に変更しない規則の正本は [data-integrity.md](../../docs/design/data-integrity.md) の「蓄積データ — 壊れているなら変更しない」。
