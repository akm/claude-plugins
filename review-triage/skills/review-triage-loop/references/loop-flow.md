...
- **未着手 (`status: pending`) は止まる理由にしない。** `review-triage-fix` が中断した合図なので、入口 (G3) でも周回の中 (J2) でも、レビューより先に `review-triage-fix` を呼んで片付ける (F1) — 直していない問題が次のレビューで再び指摘されないようにするため。
...
flowchart TD
  G0["G0: 設定と引数を読む"]
  G2{"G2: 選択待ちがあるか"}
  G3{"G3: 覆われていない採択・未着手のいずれかがあるか"}
  L1["L1: レビューを起動する"]
  L2["L2: review-triage を呼ぶ"]
  J2{"J2: 覆われていない採択・未着手のいずれかがあるか"}
...
| G3 | 判定 | J2 と同じ条件。**`review-triage` を単独で走らせた直後 (採択が残り `plans` がまだ無い) はここで「ある」になり、レビューの前に F1 を通る** — 先にレビューを走らせると、`review-triage` が次の回を追記した時点で前の回の被覆の免除 (正本は [record-schema.md](../../review-triage/references/record-schema.md) の `plan_ref` の行) が解け、記録の検査が失敗する。**未着手が残ったまま周回を起動し直したときも同じ** | — |
...
| J2 | 判定 | 全回の `findings` に、`plans` にも `plan_ref` にも覆われていない `verdict: adopted` があるか。または、全回の `plans` に `status: pending` の問題があるか | — |
| F1 | 手順 | `review-triage-fix` を呼ぶ。修正・検証・コミットはそちらが行う。**周回は、覆われていない採択を直す呼び出しと、未着手を再開するだけの呼び出しを区別して渡さない** — 記録の状態から対象 (未覆いの採択・再開する未着手) を解決し、再開だけなら計画の作り直しを飛ばして修正 (手順 7) から続けることは、`review-triage-fix` の手順 1 が行う。正本は [review-triage-fix の SKILL.md](../../review-triage-fix/SKILL.md) の手順 1 | — |
