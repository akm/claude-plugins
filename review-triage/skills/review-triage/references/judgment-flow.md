...
- **散文 (SKILL.md・references) はノード ID で参照し、順序・分岐・量化子を言い直さない。** 1 回目の試行では、散文が状態機械を言い直すたびに量化子がずれ、同型の指摘が続いた。
...
- **境界上は保留に倒す。** 決められないときの行き先は必ず保留 (H1〜H6)。却下ゲートは仮説であり、誤った却下は黙って残るため、判断できないまま却下へ流さない。
...
| D3 | 判定 | 帰結の 4 項目 — 条件 (どんな状況で起きるか) / 誰に (誰が困るか) / 何が (何が起きるか) / 気づけるか (どの関門が赤くなるか・運用中に誰が困って気づくか・気づかない) — を具体的に書けたか。指摘文は「何が悪いか」を書くが、判断に要るのは「何が壊れるか」 — 書き下ろす過程そのものが空虚な指摘をふるいにかける | 帰結の 4 項目 (`consequence`。全件) |
...
| H5 | 決着 | 保留 — 免除の条件 1 が成立するのに条件 2 の理由を書けない。検出能力が失われる型は最も見逃してはならないので、判断できないまま先へ流さない | `verdict: held`。`verdict_reason` に H5 と挙げた関門の候補 |
