...
- **既定の置き場 (`tmp/review-triage/`) は git の追跡外なので、記録 (YAML) もサマリ (`.md`) もコミットしない。** 理由は冒頭のとおり。追跡外でも、`review-triage` が修正をコミットしない (判断までが範囲) ことは変わらない。
