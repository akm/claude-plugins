...
`done-external` はそのための状態である。`done` と同じく再開の対象外だが、**状態を見ただけで「コミットで追えない修正」だと分かる**。`pending` と混同されないための区別で、`pending` のまま残すと次の `review-triage-fix` が再開対象として拾い、そのたびに人間が判断することになる。
