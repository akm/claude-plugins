...
記録の置き場はリポジトリごとに決める (既定は `tmp/review-triages/`)。**既定の置き場は git の追跡外で、記録とサマリはコミットしない** — `tmp/` が `.gitignore` にあることを前提にする。プラグインは配布物なので、この前提は利用者のリポジトリごとに確かめる: **既定の置き場を使うとき、置き場が `git check-ignore` で無視されていなければ、スキルは `.gitignore` に `tmp/` を足してコミットしてから再実行するよう案内して止まる。スキルは利用者の `.gitignore` を変えない** (`--gen-config` が設定の不在で案内して止まるのと同じ流儀。変えると、未コミットの変更が残るか、スキルが利用者のリポジトリにコミットを作ることになる)。設定の `record_dir` で置き場を明示したリポジトリは追跡内に置く意図とみなし、案内せず、下の「コミット」の節のとおり扱う (手順の側はこの文を参照する) — 記録はブランチの作業中に判断の材料として使うもので、PR がマージされた後は参照されない。コミットして履歴に残しても読まれず、コミットの数だけが増える (実測)。履歴に残したいリポジトリは、設定の `record_dir` を追跡内のパスにする (そのときのコミットの分け方は下の「コミット」の節)。置き場と検査コマンドの設定は [project-config.md](project-config.md) が定める。機械検査が見る項目の一覧は同梱ツールの [README](../../../tools/triagecheck/README.md) の「何を検査するか」が正本で、この文書では該当するキーの箇所で個別に触れるに留める。
