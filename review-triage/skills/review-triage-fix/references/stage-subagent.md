...
- 段 1 は手順 4 の末尾の「立案者の選択」を行わない — 回の番号と回数の閾値 N を比べず、人間に尋ねず、問題を `status: awaiting-human` にしない。段 1 の結果はすべて `status: investigated` で返す (立案者の選択はセッション側が行う)。**束ね方や直し方に人間の判断が要ると思ったときも選択待ちにせず**、`investigated` で返して、その判断の材料は報告の「できなかったこと」に書く。**材料を記録に書かない** — `investigation` のキーは調べた範囲と含めた箇所・含めなかった箇所だけで (正本は [record-schema.md](../../review-triage/references/record-schema.md) の調査の表)、判断の材料を置く欄が無い。選択待ちにするかどうかは段 2 が決める。
