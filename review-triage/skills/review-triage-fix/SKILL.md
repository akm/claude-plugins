...
description: review-triage が採択したレビュー指摘を、原因で束ねて問題単位で直す。指摘の数だけコミットを作らず、同じ原因のものを 1 つの問題にまとめてから順に直す。「採択した指摘を直して」「review-triage-fix を実行して」のような依頼で使う。引数に記録 YAML のパスを渡すとその記録の最後の回を読み、渡さなければ現在のブランチの記録を使う。
---
...
1. **入力を読む**: 引数の記録 YAML、無ければ現在のブランチの記録 (`<記録の置き場>/<ブランチ名>.yaml`。置き場は設定の `record_dir`) を読む。
   - **最後の回**の `findings` から `verdict: adopted` の指摘を取る。
