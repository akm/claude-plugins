...
**ラッパー経由なら、どこから叩いてもシンボリックリンク経由でも同じ置き場を検査する**
...
**在り処の不在は、経路を問わず同じ検査で報告する。** `-install-wrapper` でも、焼き込む `-record-dir` / `-judgment-flow` が無ければエラーにして生成しない。値はラッパーに固定されるので、生成時に通すと**そのラッパーは以後叩くたびに必ず失敗する**。綴りの誤り・空に展開された変数 (`$(REVIEW_TRIAGE_ROOT)` が空のときの `/skills/...` など) はここで弾かれる。
...
- **`$PWD` は使わない** — 理由は上の「パスの渡し方」のとおり。基準にすると叩く場所で
  検査対象が変わる。
...
| 配置 | サポート |
| --- | --- |
| ラッパーを直接叩く (どのディレクトリからでも) | ✓ |
| シンボリックリンク経由で叩く (多段・相対も) | ✓ |
| 経路のディレクトリがシンボリックリンク | ✓ |
| リポジトリごと移動・再クローンする | ✓ |
| **ハードリンクを張って、そこから叩く** | **✗ しない** |

**サポートしない配置は、弾かれるのではなく、警告なく合格を返す。** ハードリンク経由で叩くと、
