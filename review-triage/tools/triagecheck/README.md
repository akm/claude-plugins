...
**在り処を指定したのに対象が無いなら、問題として報告して非 0 で終わる。** 指定は「そこを検査せよ」という意思表示なので、報告せずに通すと**検査が走って合格したのか、そもそも走らなかったのかを区別できない**。プラグイン側がファイルを移動・改名したとき、利用側の CI は何も変えていないのに守りだけが外れる。
...
**環境変数 `CLAUDE_PLUGIN_ROOT` が設定された環境 (Claude Code のフックやスキルの中、それを引き継いだ CI) でも通ること。** run を呼ぶテストは共通の入口 (`withRunGlobals`) でこの変数をクリアするので、環境に依存しない。守れているかは次で確かめる (この確認を機械的に行う関門は無い)。
