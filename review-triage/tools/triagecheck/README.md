...
なお**空のパス (空文字・空白だけの `-judgment-flow "   "`・ゼロ幅スペースなど不可視の文字だけの値) を明示指定した場合は、検査する場所を決められないのでその場でエラーにする。** `-record-dir ""` だけは「空のパスの検査」ではなく必須の検査で弾かれる (省略と区別できないため)。

**在り処の不在は、経路を問わず同じ検査で報告する。** `-install-wrapper` でも、焼き込む `-record-dir` / `-judgment-flow` が無ければエラーにして生成しない。値はラッパーに固定されるので、生成時に通すと**そのラッパーは以後実行するたびに必ず失敗する**。綴りの誤り・空に展開された変数 (`$(REVIEW_TRIAGE_ROOT)` が空のときの `/skills/...` など) はここで弾かれる。
...
**サポートしない配置は、弾かれるのではなく、警告なく合格を返す。** ハードリンク経由で実行すると、
