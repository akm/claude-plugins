...
基準に使うと検査対象が黙ってすり替わる。知り得ないものを推測するより、知っている側に書かせる。
...
`-install-wrapper` が生成するラッパーは `-current-dir "$PWD"` を渡すので、
**ラッパー経由なら相対パスをそのまま使える。**

### 対象が見つからないとき

**在り処を指定したのに対象が無いなら、問題として報告して非 0 で終わる。** 指定は「そこを検査せよ」という意思表示なので、黙って通すと**検査が走って合格したのか、そもそも走らなかったのかを区別できない**。プラグイン側がファイルを移動・改名したとき、利用側の CI は何も変えていないのに守りだけが外れる。
...
なお**空のパス (`-record-dir ""` など) を明示指定した場合は、検査する場所を決められないのでその場でエラーにする。**
...
145 件 (部分テストを含む数。`go test -v ./... | grep -c 'PASS:'` で数える)。**テストを増減させたら、この行はこのコマンドで数え直す** — 記憶で書くと実測とずれる。
