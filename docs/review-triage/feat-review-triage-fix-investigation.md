...
| 問題 | 原因 | 含む指摘 | 修正方法 | 順 | 状態 | 証拠 (SHA / URL) |
| --- | --- | --- | --- | --- | --- | --- |
| P1 | 回 1 の P2 で investigation.md:5 を直したとき、同じ冒頭の節の 2 行上にある 包括文 (3 行目) を読み直さなかった。型 B の「節の記述を変えたら節全体を 読み直す」を、直した行の近傍にだけ当てて冒頭の包括文に当てなかった | #1 | investigation.md:3 の「コミット前の検証は verification.md が持つ」を 「検証 (型 A〜F) は verification.md が持つ」に改める。案内の文にフェーズを 書かなければ、5 行目や正本が変わっても 3 行目が古びる余地が無くなる。 直した後に冒頭の節 (1〜5 行目) 全体と、型を参照する 50・54・63 行目を読み直す | 1 | 済 | `f0d3cf4` |

- **P1 の調査**: 範囲: 両スキルの *.md で verification.md を参照する文 8 箇所を全部読み、 「コミット前」「書いている最中」「修正の後」で grep。このブランチで 足した・変えた冒頭 (見出し前) の包括文 8 文を読み直した / 含めなかった: verification.md:1 の題「修正の後の検証」は 11 行目の「書いている最中」と緊張があるが、正本自身の既存の記述で原因が別なので含めない; SKILL.md:37「各コミットの前に…型の検証を通す」は verification.md:11 後半 (コミット前にすべて確かめたかを見る) と同じ意味の既存の文で、整合している; committing.md:37 は「コミット前にかかるもの (B・D・E)」と正本どおりに絞っており直す必要が無い; トリアージ記録 YAML の逐語引用は過去の回として書き換えない
